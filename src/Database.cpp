...
int Database::getNextWallSectionTag() {
    ...
    for(auto& [fst, snd] : wall_section_pool)
        if(fst > tag) tag = fst;
...
int Database::getNextFrameSectionTag() {
    ...
    for(auto& [fst, snd] : frame_section_pool)
        if(fst > tag) tag = fst;
...
bool Database::removeNode(const int T) {
    ...
    while(I != element_pool.end()) {
        if(I->second.encoding.count(T) > 0)
            I = element_pool.erase(I);
        else
            ++I;
    }
...
bool Database::removeWallSection(const int T) {
    auto I = element_pool.begin();
    while(I != element_pool.end()) {
        if(I->second.type == Element::Type::Wall && I->second.section_tag == T)
            I = element_pool.erase(I);
        else
            ++I;
    }
...
void Database::changePosition(const int tag, QVector3D&& position) {
    if(node_pool.find(tag) != node_pool.end()) node_pool[tag].position = position;
}

void Database::changeFixity(const int tag, QVector<bool>&& fixity) {
    if(node_pool.find(tag) != node_pool.end()) node_pool[tag].fixity = fixity;
}

void Database::changeLoad(const int tag, QVector<double>&& load) {
    if(node_pool.find(tag) != node_pool.end()) node_pool[tag].load = load;
}

void Database::changeMass(const int tag, const double mass) {
    if(node_pool.find(tag) != node_pool.end()) node_pool[tag].mass = mass;
}

void Database::changeDisplacement(const int tag, QVector<double>&& displacement) {
    if(node_pool.find(tag) != node_pool.end()) node_pool[tag].displacement = displacement;
}

void Database::changeSection(const int ele, const int sec) {
    if(element_pool.find(ele) == element_pool.end()) return;

    if(element_pool.at(ele).type == Element::Type::Wall) {
        if(wall_section_pool.find(sec) == wall_section_pool.end()) return;
    } else if(frame_section_pool.find(sec) == frame_section_pool.end())
        return;
...
void Database::splitElement(const int tag, const int segment) {
    ...
    if(t_element.encoding.size() < 2) return;
...
bool Database::saveModel(const QString& file_name) {
    ...
    auto wall_size = 0;
    for(auto& [fst, snd] : element_pool) {
        if(snd.type == Element::Type::Frame)
            frame_size++;
        else if(snd.type == Element::Type::Brace)
            brace_size++;
        else if(snd.type == Element::Type::Wall)
            wall_size++;
    }
    output << frame_size << ' ';
    output << brace_size << ' ';
...
template<> bool Database::add<Database::Element>(const int tag, Element&& obj) {
    for(auto& I : obj.encoding)
        if(node_pool.find(I) == node_pool.end()) return false;
